...
function ArticleAuthorButtons({ body, description, slug, tagList, title }) {
  ...
  const handleClick = () => {
    if (!isAuth) return alert('You need to login firstt')
  ...
  return (
    <>
      <button className="btn btn-sm" style={{ color: '#d00' }} onClick={handleClick}>
        <i className="ion-trash-a"></i> Delete Article
      </button>{' '}
      <button className="btn btn-sm" style={{ color: '#777' }}>
        <Link className="nav-link" state={{ body, description, tagList, title }} to={`/editor/${slug}`}>
